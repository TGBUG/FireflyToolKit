# The web server and TLS layer belong to the hosting panel, not to Fireflyer
...
So Fireflyer does not install nginx configuration and does not run Certbot. `bootstrap-vps` creates the release directory, creates the `current` symlink, installs the deploy script, and stops there. The site is created once by hand in the panel UI with its document root pointed at `current`.
