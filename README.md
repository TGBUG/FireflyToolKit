...
1. **镜像** Firefly 的完整历史，但**只推 `master`**（外加 tag）——空仓库里第一个被推上去的分支会成为默认分支；而把主题那 14 条分支一次性全推上去，会让 GitHub 自己挑一个，实测它挑中的不是 `master`
2. 写入工具文件并推送
3. 禁用主题自带的 `build.yml` 和 `deploy.yml`，并**取消它自己那次推送触发的运行**。禁用不会中止已经排队的运行，而那些正是最贵的
   - GitHub **注册 workflow 是异步的**：推送刚结束时一条都没有，几秒之后才出现（实测：立即查是 0，稍等就是全部）。脚本会轮询最多 **2 分钟**等它们出现
   - 如果两分钟都没等到，它会明确告诉你**去 Actions 页面手动把这两个 Disable 掉**，而不是含糊地报一句「没有可禁用的」。这种情况下的后果是上游 CI 继续在每次推送时消耗额度，所以别略过这句话
